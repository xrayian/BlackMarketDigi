...
export function MarketPanel() {
  ...
  const handleToggleCard = (card: ClientCard) => {
    if (selectedCardIds.length >= 5 && !selectedCardIds.includes(card.id))
      return;
    soundManager.playCardSlide();
  ...
  if (activeMerchantId === null && isSheriff) {
    const nonSheriffPlayers = Array.from(playersMap.values()).filter(
      (p) => !p.isSheriff,
    );
    content = (
      ...
      <div className="flex flex-col items-center w-full max-w-4xl gap-2">
        ...
        <div className="flex items-center gap-4 mt-1">
          ...
          <motion.button
            ...
            className={`px-7 py-2.5 rounded-xl font-display font-black text-xs md:text-sm tracking-wider uppercase cursor-pointer border transition-colors ${
              selectedCardIds.length >= 1
                ? "bg-gradient-to-r from-[#3e2723] via-[#4e342e] to-[#2e1c18] text-[#d7ccc8] border-[#5d4037] shadow-[0_0_20px_rgba(62,39,35,0.5)]"
                : "bg-gradient-to-r from-gold-dark via-gold to-gold-light text-walnut-bg border-gold-light shadow-[0_0_20px_rgba(212,168,75,0.4)]"
            }`}
          >
            {selectedCardIds.length >= 1 ? "Discard" : "Keep Hand"}
          </motion.button>
        </div>
      </div>
    );
  } else if (
    activeMerchantId !== localPlayerId &&
      ...
      >
        ...
        <div className="absolute top-3 left-6 hidden sm:flex items-center gap-4 text-xs font-display">
          ...
          >
            ...
            <span className="text-[10px] text-gold-muted underline ml-0.5">
              view
            </span>
          </button>
        </div>
